...
export async function resetDb() {
  ...
  await getDb().execute(sql`TRUNCATE commission_entries, payments, cash_handovers, employee_days_off, employee_weekly_off, trip_legs, team_members, teams, package_sessions, visit_items, visit_specialists, visits, order_lines, order_moderator_changes, orders, customer_addresses, customers, package_components, packages, services, service_categories, sessions, account_invites, audit_log, salary_records, users, employees, app_settings RESTART IDENTITY CASCADE`)
}
