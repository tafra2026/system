...
describe('orders list filters', () => {
  it('filters by operational date and shows the first visit time', async () => {
    ...
    const onlyA = (await listOrders(ctx.mod.actor, { from: '2026-10-01', to: '2026-10-02' })).rows
    expect(onlyA.map((o) => o.id)).toEqual([a.id])
    const fromB = (await listOrders(ctx.mod.actor, { from: '2026-10-03' })).rows
    ...
    // New: search by district and phone suffix, payment state, awaiting a driver, sorting, pagination.
    expect((await listOrders(ctx.mod.actor, { q: 'الروضة' })).total).toBe(2)
    expect((await listOrders(ctx.mod.actor, { q: customer.phoneE164.slice(-4) })).total).toBe(2)
    expect((await listOrders(ctx.mod.actor, { payment: 'unpaid' })).total).toBe(2)
    const awaiting = await listOrders(ctx.mod.actor, { awaitingDriver: true })
